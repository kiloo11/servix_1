...
| `UPDATE_REPO` | `kiloo11/servix_1` | Репозиторий GitHub, по релизам которого проверяются обновления. |
| `GITHUB_TOKEN` | пусто | Токен для GitHub API — нужен только приватному репозиторию или при упоре в лимит запросов. |
| `DOCKER_SOCKET` | `/var/run/docker.sock` | Путь к docker-сокету, через который запускается обновление. |
| `UPDATE_CONTAINER` | hostname | Имя или id контейнера панели. По умолчанию берётся hostname — в Docker это id контейнера. |
| `UPDATE_HELPER_IMAGE` | `containrrr/watchtower:latest` | Образ, который пересоздаёт контейнер панели при обновлении. |
