<template>
  <section class="view active">
    ...
    <div class="stats-grid">
      ...
      <article class="stat-card">
        <span>{{ app.t("pnl.cardCostMonth") }}</span>
        <strong>{{ app.pnlCostMonthDisplay }}</strong>
        <small v-if="app.settings.currency !== 'RUB'" class="stat-card-sub">≈ {{ app.pnlCostMonthRubDisplay }}</small>
      </article>
    ...
    <AccordionRoot type="single" collapsible default-value="expenses">
      <AccordionItem v-if="app.botRevenue.configured" value="bot-payments" class="chart-panel table-panel">
        <AccordionHeader as="div">
          <AccordionTrigger class="table-collapsible-trigger">
            <div>
              <h2>{{ app.t("pnl.botPaymentsTitle") }}</h2>
              <span class="stat-card-sub">{{ app.tc("payment", app.botRevenue.count) }}</span>
            </div>
      ...
      <AccordionItem value="expenses" class="chart-panel table-panel">
        <AccordionHeader as="div">
          <AccordionTrigger class="table-collapsible-trigger">
            <div>
              <h2>{{ app.t("pnl.tableTitle") }}</h2>
              <span class="stat-card-sub">{{ app.t("stats.shownOf", { shown: app.pnlFilteredRows.length, total: app.tc("record", app.pnlRows.length) }) }}</span>
            </div>
            <ChevronDownIcon class="table-collapsible-chevron" :size="18" />
          </AccordionTrigger>
        </AccordionHeader>
...
export default {
  components: { AccordionContent, AccordionHeader, AccordionItem, AccordionRoot, AccordionTrigger, AppSelect, AppSelectItem, ChevronDownIcon, ChevronLeftIcon, ChevronRightIcon },
  props: {
            ...
            tooltip: {
              ...
              callbacks: {
                ...
                label: (item) => {
                  ...
                  return [
                    `${this.app.t("pnl.monthlyRevenue")}: ${this.app.formatMoney(row.revenue, currency)}`,
                    `${this.app.t("pnl.monthlyCost")}: ${this.app.formatMoney(row.cost, currency)}`,
                    `${this.app.t("pnl.monthlyNet")}: ${this.app.formatMoney(row.net, currency)}`
                  ];
                }
              }
            }
