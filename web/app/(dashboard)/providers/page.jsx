...
// Ported from src/views/ProvidersView.vue + App.vue's providerTypeGroups
// computed and openProvider() method. Extended with a "no records" bucket —
// providerTypeGroups only ever shows a provider under the asset types it
// actually has records in (ported as-is from the original), which silently
// hid freshly created providers with nothing attached to them yet.
    ...
    return (
      <article key={provider.id} className="provider-card">
        ...
        <div className="provider-asset-list">
          {items.map((asset) => (
            <span key={asset.id} className="stat-card-sub provider-asset-name">
              {asset.name}
            </span>
          ))}
        </div>
