...
// now the "Overview" tab of the merged Finance section. botRevenue/botRevenueMonthly
// come from DataContext (loaded once at app boot) instead of this component's own
// fetch-on-mount — the old page re-fetched the same /api/bot/revenue endpoints
// DataContext already holds, purely because each Next.js route mounted fresh.
export default function FinanceOverviewTab() {
  const { t, tc, locale } = useLocale();
  const { meta } = useAuth();
  const { assets, botRevenue, botRevenueMonthly } = useData();
